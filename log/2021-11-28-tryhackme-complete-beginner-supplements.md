...
Cookies can be accessed in JavaScript via [document.cookie](https://developer.mozilla.org/docs/Web/API/Document/cookie).
...
Remember that JavaScript recognizes single quote, double quotes, *and* back ticks all as valid string quoting characters!
...
[Netcat](../notes/netcat.md) can connect over UDP instead of TCP by providing the `-u` flag. I suspect that this may be less than reliable though for anything other than connections on the local network.
